...
// Login Button for Doctor
    private void dLoginBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_dLoginBtnActionPerformed
        ...
        try {
//            String sql = "select username, password from doctor where username='" + user + "','" + pass + "'";
            // String sql = "SELECT username, password from doctor WHERE username=?";

             
            prp = connection.prepareStatement("SELECT username, password FROM doctor WHERE username=?", ResultSet.TYPE_SCROLL_INSENSITIVE, 
            ResultSet.CONCUR_UPDATABLE);
        
            // prp = connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ...
            if (Hospital.hashPassword(pass).equals(result.getString("password"))) {
